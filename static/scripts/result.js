document.addEventListener("DOMContentLoaded", function () {
  ...
  const pdfUrl =
    urlParams.get("pdf") ||
    (urlParams.get("pdf_file")
      ? `/preview/${urlParams.get("pdf_file")}`
      : null);
  const latexUrl =
    (urlParams.get("latex") ? `/download/${urlParams.get("latex")}` : null) ||
    (urlParams.get("latex_file")
      ? `/download/${urlParams.get("latex_file")}`
      : null);

  const pdfDownloadUrl =
    (urlParams.get("pdf") ? `/download/${urlParams.get("pdf")}` : null) ||
    urlParams.get("pdf_download") ||
    (urlParams.get("pdf_file")
      ? `/download/${urlParams.get("pdf_file")}`
      : null);

  if (pdfUrl) {
    const pdfViewer = document.getElementById("pdfViewer");
    pdfViewer.innerHTML = `<iframe src="/preview/${urlParams.get(
      "pdf"
